describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for(let i = 0; i < prodItemsData.length; i++) {
      ...
      if (!value.title || value.title.length === 0) { allArePopulated = false; }
      if (!value.price || value.price.length === 0) { allArePopulated = false; }
      if (!value.image || value.image.length === 0) { allArePopulated = false; }
    }
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const shadowRoot = await prodItem.evaluateHandle(el => el.shadowRoot);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (const item of prodItems) {
      const shadow = await item.evaluateHandle(el => el.shadowRoot);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allHaveRemove = await page.$$eval('product-item', items =>
      items.every(item => {
    ...
    expect(allHaveRemove).toBe(true);
    expect(cartCount).toBe('20');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allBackToAdd = await page.$$eval('product-item', items =>
      items.every(item => {
        const btn = item.shadowRoot.querySelector('button');
